...
if (is_array($modelName)) {
    ...
    $json = json_decode(file_get_contents($modelPath . $modelName.'/index.json'), 1);
} else {
    $json = json_decode(file_get_contents($modelPath . $modelName.'/index.json'), 1);
...
foreach ($json['textures'] as $k => $texture)
	$json['textures'][$k] = $modelPath . $modelName . '/' . $texture;

$json['model'] = $modelPath . $modelName.'/'.$json['model'];
if (isset($json['pose'])) $json['pose'] = $modelPath . $modelName.'/'.$json['pose'];
...
if (isset($json['motions']))
    foreach ($json['motions'] as $k => $v) foreach($v as $k2 => $v2) foreach ($v2 as $k3 => $motion)
        if ($k3 == 'file' || $k3 == 'sound') $json['motions'][$k][$k2][$k3] = $modelPath . $modelName . '/' . $motion;

if (isset($json['expressions']))
    foreach ($json['expressions'] as $k => $v) foreach($v as $k2 => $expression)
        if ($k2 == 'file') $json['expressions'][$k][$k2] = $modelPath . $modelName . '/' . $expression;
