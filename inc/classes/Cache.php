...
EOS;
    ...
    public static function update_database($type) {
        global $wpdb;
        if ($type != 'screen' && $type != 'post')
            return '$type:' . $type . ' invalid.';
        $sakura_table_name = $wpdb->base_prefix . 'sakura';
        $img_domain = akina_option($type . '_cover_link') ? akina_option($type . '_cover_link') : get_template_directory() . '/manifest';
        $manifest = file_get_contents($img_domain . "/manifest.json");
        if ($manifest) {
            $manifest = array(
                "mate_key" => $type . "_manifest_json",
                "mate_value" => $manifest
            );
            $time = array(
                "mate_key" => $type . "_json_time",
                "mate_value" => date("Y-m-d H:i:s", time())
            );

            $wpdb->query("DELETE FROM  $sakura_table_name WHERE `mate_key` = '" . $type . "_manifest_json'");
            $wpdb->query("DELETE FROM  $sakura_table_name WHERE `mate_key` = '" . $type . "_json_time'");
            $wpdb->insert($sakura_table_name, $manifest);
            $wpdb->insert($sakura_table_name, $time);
            $output = "manifest.json has been stored into database(". $type .").";
        } else {
