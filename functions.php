 ...
 *
 * 标准的 lazyload 输出头像
 * <?php echo str_replace( 'src=', 'src="'.get_template_directory_uri().'0.1/img/svg/loader/index.ajax-spinner-preloader.svg" onerror="imgError(this,1)" data-src=', get_avatar( $comment->comment_author_email, '80', '', get_comment_author(), array( 'class' => array( 'lazyload' ) ) ) ); ?>
 *
 * 如果不延时是这样的
			...
			<div class="contents">
				<div class="comment-arrow">
					<div class="main shadow">
						<div class="profile">
							<a href="<?php comment_author_url();?>" target="_blank" rel="nofollow"><?php echo str_replace('src=', 'src="'.get_template_directory_uri().'0.2/img/svg/loader/trans.ajax-spinner-preloader.svg" onerror="imgError(this,1)" data-src=', get_avatar($comment->comment_author_email, '80', '', get_comment_author(), array('class' => array('lazyload')))); ?></a>
						</div>
...
function get_author_class($comment_author_email, $user_id)
{
    ...
    {
        echo '<span class="showGrade0" title="Lv0"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_0.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 6 && $author_count < 10) {
        echo '<span class="showGrade1" title="Lv1"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_1.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 10 && $author_count < 20) {
        echo '<span class="showGrade2" title="Lv2"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_2.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 20 && $author_count < 40) {
        echo '<span class="showGrade3" title="Lv3"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_3.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 40 && $author_count < 80) {
        echo '<span class="showGrade4" title="Lv4"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_4.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 80 && $author_count < 160) {
        echo '<span class="showGrade5" title="Lv5"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_5.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 160) {
        echo '<span class="showGrade6" title="Lv6"><img src="'.get_template_directory_uri().'/img/Sakura/images/level/level_6.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    }
...
function custom_html()
{
    ...
    echo 'jQuery("body").prepend("<div class=\"loading\"><img src=\"'.get_template_directory_uri().'/img/Sakura/images/login_loading.gif\" width=\"58\" height=\"10\"></div><div id=\"bg\"><img /></div>");' . "\n";
    ...
    if (($parent_id != '') && ($spam_confirmed != 'spam') && ($admin_notify != '0') && (!$mail_notify)) {
      ...
      box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.18);">
        <header style="overflow: hidden;">
            <img style="width:100%;z-index: 666;" src="'.get_template_directory_uri().'/img/other/head.jpg">
        </header>
      ...
      <div style="text-align: center;">
          <img src="'.get_template_directory_uri().'/img/other/hr.png" alt="hr" style="width:100%;
...
';
        ...
        $message = str_replace("{{", '<img src="'.get_template_directory_uri().'/img/bili/hd/ic_emoji_', $message);
...
function comment_picture_support($content)
{
    ...
    $content = str_replace('[img]', '<br><img src="'.get_template_directory_uri().'0.2/img/svg/loader/trans.ajax-spinner-preloader.svg" data-src="', $content);
    $content = str_replace('[/img]', '" class="lazyload comment_inline_img" onerror="imgError(this)"><br>', $content);
    $content = str_replace('[/img^img]', '" class="lazyload comment_inline_img" onerror="imgError(this)"><img src="'.get_template_directory_uri().'0.2/img/svg/loader/trans.ajax-spinner-preloader.svg" data-src="', $content);
    return $content;
}
...
{
    return get_template_directory_uri().'/img/Sakura/images/smilies/' . $img;
}
...
function push_smilies()
{
    ...
    foreach ($wpsmiliestrans as $k => $v) {
        $Sname = str_replace(":", "", $k);
        $Svalue = $v;
        $return_smiles = $return_smiles . '<span title="' . $Sname . '" onclick="grin(' . "'" . $Sname . "'" . ')"><img src="'.get_template_directory_uri().'/img/Sakura/images/smilies/' . $Svalue . '" /></span>';
    }
...
function push_bili_smilies()
{
    ...
    for ($i = 0; $i < count($name); $i++) {
        ...
        // 选择面版
        $return_smiles = $return_smiles . '<span class="emotion-secter emotion-item emotion-select-parent" onclick="grin(' . "'" . $name[$i] . "'" . ',type = \'Math\')" style="background-image: url('.get_template_directory_uri().'/img/bili/hd/ic_emoji_' . $name[$i] . '.png);"><div class="img emotion-select-child" style="background-image: url('.get_template_directory_uri().'/img/bili/' . $name[$i] . '.png);
        ...
        "></div></span>';
        // 正文转换
        $bilismiliestrans['{{' . $name[$i] . '}}'] = '<span class="emotion-inline emotion-item"><img src="'.get_template_directory_uri().'/img/bili/' . $name[$i] . '.png" class="img" style="/*background-image: url();*/
        animation-duration: ' . ($img_height / 32 * 40) . 'ms;
        animation-timing-function: steps(' . ($img_height / 32) . ');
        transform: translateY(-' . ($img_height - 32) . 'px);
        height: ' . $img_height . 'px;
...
{
    $content = str_replace("{{", '<img src="'.get_template_directory_uri().'/img/bili/hd/ic_emoji_', $content);
