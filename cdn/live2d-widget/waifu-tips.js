...
// 后端接口
live2d_settings['modelAPI']             = '/wp-content/themes/Sakura/cdn/live2d_api/';   // 自建 API 修改这里
...
function initModel(waifuPath, type) {
    ...
    if (live2d_settings.homePageUrl == 'auto')
    {
        if (window.location.port.toString() == "")
            live2d_settings.homePageUrl = window.location.protocol + '//' + window.location.hostname + '/';
        else
            live2d_settings.homePageUrl = window.location.protocol + '//' + window.location.hostname + ':' + window.location.port.toString() + '/';
    }
